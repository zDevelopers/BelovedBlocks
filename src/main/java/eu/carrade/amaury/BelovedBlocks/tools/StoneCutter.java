...
public class StoneCutter extends BelovedTool {
	...
	public boolean useableOn(Block block) {
		...
		return type == Material.STONE_SLAB || type == Material.SANDSTONE_SLAB || type == Material.RED_SANDSTONE_SLAB
				|| type == Material.QUARTZ_SLAB;
	}

	@Override
	protected List<String> getUsage() {
		return Collections.singletonList(I.t("{gray}Right-click on a double slab to smooth or carve the block."));
	}

	@Override
	protected boolean onUse(Player player, Block block) {
		if (!((Slab) block.getBlockData()).getType().equals((Slab.Type.DOUBLE))) {
			return false;
		}
