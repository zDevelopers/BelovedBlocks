...
abstract public class BelovedItem {
	 ...
	 *
	 * Used by the /bb give command, and for the permissions: - usage permission
	 * (place, remove): belovedblocks.blocks.{internalName}.use (default true);
	 * - craft permission (craft, uncraft):
	 * belovedblocks.blocks.{internalName}.craft (default true); - give
	 * permission: belovedblocks.blocks.{internalName}.give.self (default op),
	...
	public BelovedItem(String internalName, Material itemMaterial, String displayName, boolean isCraftable,
			boolean glowOnItem) {
		if (displayName != null)
	 ...
	 * this(internalName, (Material)null, config); }
	 */
	...
	protected ItemStackBuilder getItemBuilder() {
		PluginLogger.info("internal name {0}", internalName);
		return new ItemStackBuilder(itemMaterial).title(displayName).glow(glowOnItem);
		/*
		 * return new ItemStackBuilder(itemMaterialData.getItemType())
		 * .data(itemMaterialData.getData()) .title(displayName)
		 * .glow(glowOnItem);
		 */
	}
	...
	public ItemStack makeItem(int amount) {
		...
		try {
			PluginLogger.info("Item {0} Atribute = {1}", item, new Attribute(this));
	...
	public boolean is(ItemStack item) {
		if (item == null)
			return false;
		if (!item.getType().equals(itemMaterial))
			return false;
	 ...
	 *            The player.
	 * @param self
	 *            {@code true} if the receiver is the same player as the giver.
	 *
	 * @return {@code true} if allowed.
	 */
	 ...
	 * @param user
	 *            The player to check.
	 * @param subPermissionNode
	 *            The sub permission node.
	 * @param extra
	 *            An extra permission string that will be appended to the final
	...
	private Boolean isAllowed(CommandSender user, String subPermissionNode, String extra) {
		String permissionString = "belovedblocks." + subPermissionNode + "." + getItemTypeString() + "."
				+ getInternalName();
		if (extra != null)
	...
	/**
	 * Returns the string identifier for the type (block, tool, …) of this
	 * item.
	 * 
	 * @return the string identifier for the type (block, tool, …) of this
	 *         item.
	 */
